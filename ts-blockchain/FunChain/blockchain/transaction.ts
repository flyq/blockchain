import * as CryptoJS from 'crypto-js'; // SHA265
import * as ecdsa from 'elliptic'; // 椭圆曲线加密
...
// 每个Transaction的ID都是hash得到，为了不被篡改交易数据
...
// 验证某一个Transaction结构是否合法：Transaction变量类型，数据是否篡改，TxIns是否有效（签名/是否被使用），输入输出是否相等
const validateTransaction = (transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {

    // Transaction里面的变量类型验证
    if (!isValidTransactionStructure(transaction)) {
        return false;
    }

    // Transaction的ID验证，也就是验证Transaction里面的数据有没有被篡改。
...
// 验证第一个Transaction结构，就是系统发给矿工奖励的那个
...
// 检查TxIn结构里面1、签名是有效的。2、里面的UTXO没有被使用
...
// 定义CoinbaseTransaction，就是奖励矿工的那笔
